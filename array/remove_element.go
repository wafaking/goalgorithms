...
func removeElement2(nums []int, val int) int {
	...
	for left < right {
		if nums[left] == val {
			nums[left], nums[right-1] = nums[right-1], 0
			right--
		} else {
			left++
		}
